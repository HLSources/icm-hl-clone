...
void EV_FireMP5( event_args_t *args )
{
	...
	shell = gEngfuncs.pEventAPI->EV_FindModelIndex ("models/pshell.mdl");// brass shell
	...
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/mac10-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
...
//	{
//		case CROSS_DOT:
//			gHUD.icm_crosshair = CROSS_NOCROSS;
//			return;
//		case CROSS_NOCROSS:
//			gHUD.icm_crosshair = CROSS_DOT;
//			return;
//	}
	...
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/awp1.wav", 1, ATTN_NORM, 0, 93 + gEngfuncs.pfnRandomLong(0,0xF) );
	...
	if ( tr.fraction < 1.0 )
	{
		...
		else if ( pe->rendermode == kRenderNormal ) 
		{
			...
			vec3_t vBoltAngles;
			int iModelIndex = gEngfuncs.pEventAPI->EV_FindModelIndex( "models/rpgrocket.mdl" );
...
//	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_ITEM, "weapons/tmp-1.wav", 0.7, ATTN_NORM, 0, PITCH_NORM );
...
//		{
//		case 0:
//			gEngfuncs.pEventAPI->EV_WeaponAnimation( RPG_FIRE1, 1 ); break;
//		case 1:
//			gEngfuncs.pEventAPI->EV_WeaponAnimation( RPG_FIRE2, 1 ); break;
//		case 2:
//			gEngfuncs.pEventAPI->EV_WeaponAnimation( RPG_FIRE3, 1 ); break;
//		}
...
void EV_FireRpg( event_args_t *args )
{
	...
	shell = gEngfuncs.pEventAPI->EV_FindModelIndex ("models/rshell.mdl");// brass shell Das Model der Patronenhülse die ausgeworfen wird
	
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		//RPG_FIRE1 ist die Animation die abgespielt werden soll
		gEngfuncs.pEventAPI->EV_WeaponAnimation( RPG_FIRE1 + gEngfuncs.pfnRandomLong(0,2), 2 ); 
	}
	
	//Positionsangabe an der die Hülse ausgeworfen wird
	EV_GetDefaultShellInfo( args, origin, velocity, ShellVelocity, ShellOrigin, forward, -right, up, 15, -8, -10 );
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); //lässt die Patronenhülse herrausfallen
