...
//
// $Workfile:     $
// $Date: 2003/02/16 13:25:33 $
//
//-----------------------------------------------------------------------------
// $Log: vgui_ScorePanel.cpp,v $
// Revision 1.2  2003/02/16 13:25:33  arsh0r
// *** empty log message ***
//
// Revision 1.1.1.1  2002/10/14 16:16:43  arsh0r
// no message
//
//
// $NoKeywords: $
		...
		for(int col=0; col < NUM_COLUMNS; col++)
		{
			...
			else
			{
				...
				case COLUMN_LATENCY:
					...
					if (g_PlayerInfoList[ m_iSortedRows[row] ].ping < 5 )
						sprintf(sz, "Host");
					else if (g_PlayerInfoList[ m_iSortedRows[row] ].ping == 5 )
						sprintf(sz, "Bot");
					else sprintf(sz, "%d", g_PlayerInfoList[ m_iSortedRows[row] ].ping );
					break;
				default:
					break;
				}
