...
{
	SetLookAtBiasVec( GetLookAtBiasVec() + Vector( RANDOM_FLOAT(-.1,.1), RANDOM_FLOAT(-.1,.1), RANDOM_FLOAT(-.1,.1) ) );
	SetLookAtBiasVec( GetLookAtBiasVec().Normalize() );
	SetLookAtVec( GetLookAtVec().Normalize() );
	SetLookAtBiasVec( GetLookAtBiasVec() * ( 1 - ( Stats.GetTraitAccuracy() / 100 ) ) * RANDOM_FLOAT(0,1) );
...
void CBaseBot::AimWander( void )
{
	...
	Vector newLookAtBiasVec = GetLookAtBiasVec() + Vector( RANDOM_FLOAT(-.1,.1), RANDOM_FLOAT(-.1,.1), RANDOM_FLOAT(-.1,.1) );
...
void CBaseBot::HandleMovement( void )
{
	...
	Vector TrueLookAtVec = GetLookAtVec()
						...
						+ Memory.GetPrev2LookAtVec()
						+ Memory.GetPrev3LookAtVec()
						+ Memory.GetPrev4LookAtVec();
	...
	if (pev->v_angle.y > 180) pev->v_angle.y -=360;
	if (pev->v_angle.x > 180) pev->v_angle.x -=360;
	...
	Vector TrueDesiredVelocity = GetDesiredVelocity()
								+ Memory.GetPrev1Velocity()
								+ Memory.GetPrev2Velocity()
								+ Memory.GetPrev3Velocity()
								+ Memory.GetPrev4Velocity();
	...
	SetMoveStrafe( BotMoveSpeed - GetMoveForward() ); // put into MoveStrafe whatever speed isn't already being used by MoveForward
...
Vector CBaseBot::HandleObstacles( Vector &IdealVelocity )
{
	...
	{
		Vector Normal = UTIL_VecToAngles( tr.vecPlaneNormal ); // vector that is perpendicular to the surface from the TraceResult
		...
		if ( ( ( abs( diffWallY1 ) > abs( diffWallY2 ) ) && GetTurningDirection() == NONE )
			|| GetTurningDirection() == LEFT
			) // turn the way which takes the least effort if we weren't already turning a certain direction
		{
			SetTurningDirection( LEFT );

			DirectionHeadingAngle.y -= ( diffWallY1 * TweakYawPercent * RANDOM_FLOAT((1 - YAW_VARIATION),(1 + YAW_VARIATION)) );
		}
		else
		{
			SetTurningDirection( RIGHT );

			DirectionHeadingAngle.y -= ( diffWallY2 * TweakYawPercent * RANDOM_FLOAT((1 - YAW_VARIATION),(1 + YAW_VARIATION)) );
		}

		if ( abs( botY - DirectionHeadingAngle.y ) > ALLOWED_TURNING_CHANGE )
		{
			if ( ( botY - DirectionHeadingAngle.y ) < 0 )
			{
				DirectionHeadingAngle.y = botY - ALLOWED_TURNING_CHANGE;
			}
			else
			{
				DirectionHeadingAngle.y = botY + ALLOWED_TURNING_CHANGE;
	...
	if ( CheckPathObstructedSides( IdealVelAngle, tr, tr2 ) )
	{
		...
		{
			DirectionHeadingAngle.y -= RANDOM_FLOAT(1,5); // TODO: this could use increase (but needs smoothing)
		}
		else
		{
			DirectionHeadingAngle.y += RANDOM_FLOAT(1,5);
		}
	}
...
inline void CBaseBot::SteerGroupAlign( void )
{
...
inline void CBaseBot::SteerGroupCohesion( void )
{
	int PlayersInLocality = 0;
	float DistanceToPlayer;
...
inline void CBaseBot::SteerGroupEscort( CBaseEntity *escortTarget )
{
...
inline void CBaseBot::SteerGroupFlock( void )
{
	SteerSafeGroupAlign();
...
inline void CBaseBot::SteerGroupFollow( CBaseEntity *followTarget )
{
...
inline void CBaseBot::SteerGroupSeparation( void )
{
...
{
	if ( GetEnemy() ) // we don't want to be on a ladder during combat
	{
		pev->button |= IN_JUMP;
...
inline void CBaseBot::SteerPrimitiveWander( float WanderRate )
{
...
inline void CBaseBot::SteerPrimitiveWanderImproved( void )
{
	DIRECTION moveThisWay = ActionChooseDirection();

	int tryCounter = 0;
	//Scott:  With Change to enum DIRECTION in bot.h, logic has been simplified to:
	while ( ((GetRoamDirection() & moveThisWay )==0) && (tryCounter < 10)) //30-July-2001: correction && instead of ||
	{
	...
	{
		Vector moveThisWayVelocity = GetDesiredVelocity();

		if ( NORTHEAST == moveThisWay )
		{
			moveThisWayVelocity = Vector(1,-1,0);
		}
		else if ( NORTH == moveThisWay )
		{
			moveThisWayVelocity = Vector(1,0,0);
		}
		else if ( NORTHWEST == moveThisWay )
		{
			moveThisWayVelocity = Vector(1,1,0);
		}
		else if ( WEST == moveThisWay )
		{
			moveThisWayVelocity = Vector(0,1,0);
		}
		else if ( SOUTHWEST == moveThisWay )
		{
			moveThisWayVelocity = Vector(-1,1,0);
		}
