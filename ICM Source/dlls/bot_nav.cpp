...
int CGraph::FLoadBotNav( char *szMapName )
{
	char navFileName[MAX_PATH] = "default";

	sprintf( navFileName, "icm/nav/%s.nav ", szMapName );
...
void CGraph::slurp_array(NAV_ARRAY_TYPE space_array[NAV_ARRAY_SIZE][NAV_ARRAY_SIZE][NAV_ARRAY_SIZE], char* filename) {
...
void CGraph::SaveNavToFile( void )
{
	...
	for ( i = 0; i < NAV_ARRAY_SIZE; i++ )
	{
		for ( j = 0; j < NAV_ARRAY_SIZE; j++ )
		{
			for ( k = 0; k < NAV_ARRAY_SIZE; k++ )
			{
