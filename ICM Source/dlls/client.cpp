...
//START BOT: Scott - for better count
	CBasePlayer *pPlayer;
...
//START BOT: Scott - for better count
	pPlayer->SpawnIndex = 0; // Player will not spawn again
//END BOT
	...
	else if (FStrEq(pcmd, "addbot" ))
	{
		if (!IS_DEDICATED_SERVER())
		{
			...
			if ( NumPlayersInGameNow < MAX_BOTS )
			{
				...
				if ( gpGlobals->time >= TimeNextPossibleBotConnect )
				{
//					BotConnect(pEntity);
					BotConnect(NumPlayersInGameNow);
					TimeNextPossibleBotConnect = gpGlobals->time + BOTCONNECT_DELAY;
					CVAR_SET_FLOAT("bot_number", ((bot_number.value)+1)); // increase the number of bots allowed
				}
				else if ( TimeNextPossibleBotConnect > gpGlobals->time + BOTCONNECT_DELAY )
				{
					TimeNextPossibleBotConnect = gpGlobals->time + BOTCONNECT_DELAY;
				}
			}
			...
			{
				UTIL_ClientPrintAll( HUD_PRINTNOTIFY, "Rho-Bot supports a maximum of 32 players.\n" );
				return;
			}
		}
		else
	...
	{ // should be used when bots have a nonzero ping
		if (!IS_DEDICATED_SERVER())
		{
			for ( int i = 1; i <= gpGlobals->maxClients; i++ )
			{
				...
				{
					char serverKickCommand[128] = "removebot Some Bot\n";
	
					sprintf( serverKickCommand, "removebot \"%s\"\n", STRING( pPlayer->pev->netname ) );
	
					CLIENT_COMMAND( pEntity, serverKickCommand );
				}
			}
			CVAR_SET_FLOAT("bot_number", 0); // no bots allowed
		}
		else
	...
	else if (FStrEq(pcmd, "removebot" ))
	{
		if (!IS_DEDICATED_SERVER())
		{
			CBaseEntity *someEntity = UTIL_FindEntityByString( NULL, "netname", (char *)CMD_ARGV(1) );
	
			if ( someEntity )
			{
				//Scott: Set respawn flag, prevents respawning removed bot
				bot_respawn[someEntity->SpawnIndex].is_used = FALSE;
				someEntity->SpawnIndex = -1;
	
				someEntity->pev->health = 0;
				...
				CLIENT_COMMAND( pEntity, serverKickCommand );
				CVAR_SET_FLOAT("bot_number", ((bot_number.value)-1)); // decrease the number of bots allowed
			}
		}
		else
	...
	{
		WorldGraph.FLoadBotNav( (char *)STRING( gpGlobals->mapname ) );
	}
...
void StartFrame( void )
{
	...
	if ( NumBotsInGameNow < (bot_number.value) //Starbreaker - use cvar now
	...
	if ( ( NumBotsInGameNow > 0 && NumPlayersInGameNow == gpGlobals->maxClients)
		|| NumBotsInGameNow > (bot_number.value)
	    )
	{
			...
			if ( cvar_bot && cvar_bot[0] )
			{
				...
				if (strcmp(cmd, "addbot") == 0)
				{
					...
					else
					{
						UTIL_ClientPrintAll( HUD_PRINTNOTIFY, "Rho-Bot supports a maximum of 32 players.\n" );
						if (IS_DEDICATED_SERVER())
							printf("Rho-Bot supports a maximum of 32 players.\n");
						return;
					}
				}
				...
				else if (strcmp(cmd, "loadnav" ) == 0)
				{
					WorldGraph.FLoadBotNav( (char *)STRING( gpGlobals->mapname ) );
				}
				else if (strcmp(cmd, "savenav" ) == 0)
				{
					WorldGraph.SaveNavToFile();
				}			
			}
...
// Kamikaze sounds	
	PRECACHE_SOUND("player/hic1.wav");
	PRECACHE_SOUND("player/hic2.wav");	
	PRECACHE_SOUND("player/hic3.wav");
	PRECACHE_SOUND("player/hic4.wav");
