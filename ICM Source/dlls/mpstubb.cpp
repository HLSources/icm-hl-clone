...
// I redid the Look and BestVisibleEnemy functions so that snarks and hornets correctly tracked bots.
// rather than create a linked list and then again look through it, i've decided to make BestVisibleEnemy
// empty, and have Look set the tracking target itself. this method is actually more efficient than the
// previous, but has a narrower scope of applications.

void CBaseMonster::Look( int iDistance )
{
	float	DistanceToPlayer;
	float	NearestEnemyDistance = iDistance + 1;

	for ( int i = 1; i <= gpGlobals->maxClients; i++)
	{
		CBaseEntity *pPlayer = UTIL_PlayerByIndex( i );

		if ( pPlayer && ( pPlayer != this ) && pPlayer->IsAlive() && FInViewCone( pPlayer ) && FVisible( pPlayer ) )
		{
			DistanceToPlayer = ( pPlayer->pev->origin - pev->origin ).Length();

			if ( ( DistanceToPlayer < iDistance ) && ( DistanceToPlayer < NearestEnemyDistance ) )
			{
