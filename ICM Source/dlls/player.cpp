...
// many features in this function use v_forward, so makevectors now.
	...
	if ( (m_fKamikaze == 0) && (pev->button & IN_DUCK) && ( pev->flDuckTime > 0 ) && pev->velocity.Length() > 50 )
	{
		pev->punchangle.x = -5;
		pev->velocity = gpGlobals->v_forward * (350 * 3);
		
		switch (RANDOM_LONG(1,4)) 
		{
		case 1: 
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/hic1.wav", 1, ATTN_NORM);
			break;
		case 2: 
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/hic2.wav", 1, ATTN_NORM);
			break;
		case 3: 
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/hic3.wav", 1, ATTN_NORM);
			break;
		case 4: 
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/hic4.wav", 1, ATTN_NORM);
			break;
		}
	
		pev->velocity.z = sqrt( 2 * 800 * 100.0 );
		m_fKamikaze = 2;
		SetTouch( &CBasePlayer::KamikazeTouch );
...
#define KAMIKAZE_DAMAGE 250


void CBasePlayer::KamikazeEnd( )
{
	pev->punchangle.x = -5;
	pev->velocity = gpGlobals->v_forward * (350 * 5);
...
void CBasePlayer::KamikazeTouch( CBaseEntity* Target )
{
	 SetTouch ( NULL );     
     m_fKamikaze = 1;
	 ...
	 RadiusDamage ( pev, pev, KAMIKAZE_DAMAGE, CLASS_NONE, DMG_BLAST );
	
	 //char PrintOutText[200];
	 //hudtextparms_t hText;
	 //sprintf( PrintOutText, "%s: %s\n", STRING( pev->netname ), CVAR_GET_STRING	("lastwords") );
	 ...
	 //hText.b1 = 0;
	 //hText.a1 = 100;
	 //hText.fadeinTime = 0.05;
	 //hText.fadeoutTime = 0.5;
	 //hText.holdTime = 1;
	 //hText.fxTime = 0.5;

	 //UTIL_HudMessageAll( hText, PrintOutText );
}
...
void CBasePlayer :: UpdateClientData( void )
{
	...
	{
		int iHealth = max( pev->health, 0.0f );  // make sure that no negative health values are sent
